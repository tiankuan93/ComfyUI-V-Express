...
function previewVideo(node, file) {
  ...
  try {
    var el = document.getElementById("VEuploadVideo");
    el.remove();
  } catch (error) {
    console.log(error);
  }
  var element = document.createElement("div");
  element.id = "VEuploadVideo";
  ...
  previewWidget.videoEl.autoplay =
    !previewWidget.value.paused && !previewWidget.value.hidden;
  ...
  if (
    !params.force_size ||
    params.force_size.includes("?") ||
    params.force_size == "Disabled"
  ) {
  ...
  previewWidget.videoEl.src = api.apiURL(
...
function videoUpload(node, inputName, inputData, app) {
  const videoWidget = node.widgets.find((w) => w.name === "video_path");
  let uploadWidget;
  /*
    A method that returns the required style for the html
    */
  var default_value = videoWidget.value;
  Object.defineProperty(videoWidget, "video_path", {
    set: function (value) {
...
ComfyWidgets.VEVIDEOUPLOAD = videoUpload;

app.registerExtension({
  name: "ComfyUI-V-Express.UploadVideo",
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (
      nodeData?.name == "Load_Audio_Path_From_Video" ||
      nodeData?.name == "Load_Kps_Path_From_Video"
    ) {
      nodeData.input.required.upload = ["VEVIDEOUPLOAD"];
    }
  },
});
