...
function previewAudio(node, file) {
  ...
  try {
    var el = document.getElementById("VEuploadAudio");
    el.remove();
  } catch (error) {
    console.log(error);
  }
  var element = document.createElement("div");
  element.id = "VEuploadAudio";
  ...
  previewWidget.audioEl.addEventListener("loadedmetadata", () => {
    previewWidget.aspectRatio =
      previewWidget.audioEl.audioWidth / previewWidget.audioEl.audioHeight;
  });
  ...
  previewWidget.audioEl.autoplay =
  ...
  previewWidget.audioEl.src = api.apiURL(
...
ComfyWidgets.VEAUDIOUPLOAD = audioUpload;

app.registerExtension({
  name: "ComfyUI-V-Express.UploadAudio",
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData?.name == "Load_Audio_Path") {
      nodeData.input.required.upload = ["VEAUDIOUPLOAD"];
    }
  },
});
